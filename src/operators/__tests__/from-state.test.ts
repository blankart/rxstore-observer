...
describe( 'fromState', () => {
    test( 'fromState should yield the current state value', () => {
        ...
        dummyStore.addObserver( 'CHANGE_DUMMY_FIELD_2', pipe => pipe(
            fromState( state => state.dummyField1 ),
            mapTo( { type: 'CHANGE_DUMMY_FIELD_3', payload: 'Changed from another observer' } )
        ) )
        ...
        expect( dummyStore.getState().dummyField3 ).toBe( 'Changed from another observer' )
    } )
} )